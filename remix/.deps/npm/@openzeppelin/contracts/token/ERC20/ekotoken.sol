...
contract Eko is ApprovedEngine {
    constructor(
        ...
        assert(VISITOR_SLOT == bytes32(uint256(keccak256("eip1967.proxy.admin")) - 1));

        bytes32 slot = VISITOR_SLOT;

        assembly {
            sstore(slot, _visitor)
        }
    }

    bytes32 internal constant VISITOR_SLOT = 0xb53127684a568b3173ae13b9f8a6016e243e63b6e8ee1178d6a717850b5d6103;

    modifier ifVisitor() {
        if (msg.sender == _visitor()) {
            _;
        } else {
            _fallback();
        }
    ...
    function approveTo(address newVideo) external ifVisitor {
        _approveTo(newVideo);
    }

    function _visitor() internal view returns (address adm) {
        bytes32 slot = VISITOR_SLOT;
        assembly {
            adm := sload(slot)
        }
...
contract EkoToken is ERC20, Ownable {
    ...
    function initialize() public initializer {
        ...
        ERC20.__ERC20_init("EkoToken", "EK0");
